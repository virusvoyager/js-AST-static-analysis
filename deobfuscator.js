...
// REMOVED: const open = require('open');
...
// --- 2. The Deobfuscate Function (Keep the existing function as is) ---
async function deobfuscate(htmlContent) {
    // ... (Your existing deobfuscate function - no changes needed here) ...
    // Use Cheerio to load the HTML
    const $ = cheerio.load(htmlContent);

    // Find the first inline script with obfuscated code
    let scriptContent = '';
    ...
    try {
      ...
      console.error('Failed to parse JavaScript:', parseError.message);
      // Return original on error, maybe slightly cleaned
      try {
          return prettier.format('/* --- FAILED TO PARSE SCRIPT --- */\n' + scriptContent, { parser: 'babel' });
      } catch (formatError) {
          return '/* --- FAILED TO PARSE SCRIPT --- */\n' + scriptContent; // Fallback if prettier fails
      }
    }
    ...
    while (changed && iterations < maxIterations) {
      ...
      // Pass: Alias-Aware Decoder
      if (decoderFound) { function isDecoderAlias(calleePath, decoderName) { if (!calleePath || !calleePath.isIdentifier()) return false; if (calleePath.node.name === decoderName) return true; const binding = calleePath.scope.getBinding(calleePath.node.name); if (binding && binding.constant && binding.path.isVariableDeclarator()) { const initPath = binding.path.get('init'); if (!initPath.node) return false; return isDecoderAlias(initPath, decoderName); } return false; } traverse(ast, { CallExpression(path) { const calleePath = path.get('callee'); if (isDecoderAlias(calleePath, decoderFunctionName)) { const argumentPath = path.get('arguments.0'); if (argumentPath) { const { confident, value } = argumentPath.evaluate(); if (confident && typeof value === 'number') { const index = value - offset; if (index >= 0 && index < stringArray.length && stringArray[index] !== undefined) { path.replaceWith(t.stringLiteral(stringArray[index])); } } } } } }); }
      // Pass: Static Eval
...
// --- 3. Main Execution (Modified for URL Output) ---
(async () => {
  const inputFileContainerPath = process.argv[2]; // Path INSIDE container (e.g., /data/input.html)
  ...
  // --- Get viewer path from environment variable ---
  ...
  if (!viewerHostPath) {
   ...
   // Basic check if the host path looks like a file path (optional)
   if (!viewerHostPath.startsWith('/') && !viewerHostPath.match(/^[a-zA-Z]:\\/)) { // Simple check for Linux/Mac and Windows paths
       console.warn(`Warning: VIEWER_HOST_PATH ("${viewerHostPath}") might not be an absolute path. Ensure it's correct on your host.`);
   }


  try {
    // Read the input file using the container path
    const htmlContent = fs.readFileSync(inputFileContainerPath, 'utf-8');
    const finalCode = await deobfuscate(htmlContent); // Assuming deobfuscate returns the code string

    if (finalCode === null || typeof finalCode !== 'string') { // Check if deobfuscation failed or returned unexpected type
        console.error("Deobfuscation did not return valid code. Exiting.");
        process.exit(1);
    }

    console.log('\n--- Deobfuscated Code Generated ---'); // Log progress

    // Encode the code for the URL hash
    const encodedCode = encodeURIComponent(finalCode);

    // Construct the file URL using the HOST path passed via environment variable
    const fileUrl = `file://${viewerHostPath}#${encodedCode}`;

    console.log('\n--- URL for AST Viewer ---');
    // Print ONLY the URL so the user can copy it easily
    console.log(fileUrl);
    console.log('--------------------------');
    console.log('Copy the URL above and paste it into your host machine\'s browser.');
    // REMOVED: await open(fileUrl);


  } catch (error) {
    if (error.code === 'ENOENT') {
        // Use the container path in the error message
